...
function DisplayAll(props) {
  ...
  return dataArray.map((ele,index) => {
    const shortTitle=ele.title.length>25? `${ele.title.slice(0,25)}...`: ele.title;
    return (
      <article className="relative flex flex-col overflow-hidden rounded-lg border" key={index}>
        ...
        <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
          <div className="mb-2 flex">
            ...
            <del className="text-xs text-gray-400"> {ele.original_price} </del>
          </div>
